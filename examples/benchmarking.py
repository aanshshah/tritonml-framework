...
def generate_test_data(num_samples: int = 100):
    ...
    sentiments = ["positive", "negative", "great", "terrible", "amazing",
                  "awful"]
...
def main():
    ...
    try:
        ...
        print("\nBenchmark Summary")
        print("=" * 60)
        print(f"{'Batch Size':<12} {'Avg Latency':<15} {'P95 Latency':<15} "
              f"{'Throughput':<15}")
        print("-" * 60)
        ...
        print(f"Error: {e}")
        print("\nTo run this example:")
        print("1. Start Triton server: docker run -p 8000:8000 "
