...
class TritonConfig:
    ...
    triton_server_url: str = field(default_factory=lambda: os.environ.get("TRITON_SERVER_URL", "localhost:8000"))
    ...
    # Paths
    model_repository_path: Path = field(default_factory=lambda: Path(os.environ.get("MODEL_REPOSITORY_PATH", "./models")))
    
    # Performance settings
    instance_group: Dict[str, Any] = field(default_factory=lambda: {"kind": "KIND_CPU", "count": 1})
    ...
    def to_pbtxt(self) -> str:
        ...
        for input_name, shape in self.input_shapes.items():
            ...
            config_lines.append(f'    data_type: {self.model_config.get(f"{input_name}_dtype", "TYPE_INT64")}')
        ...
        for output_name, shape in self.output_shapes.items():
            ...
            config_lines.append(f'    data_type: {self.model_config.get(f"{output_name}_dtype", "TYPE_FP32")}')
            config_lines.append(f'    dims: {shape}')
            config_lines.append("  }")
            config_lines.append("]")
