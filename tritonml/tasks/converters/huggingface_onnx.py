...
class HuggingFaceONNXConverter(ONNXConverter):
    ...
    def __init__(self, model: AutoModelForSequenceClassification, tokenizer: AutoTokenizer, config: Dict[str, Any]):
    ...
    def quantize(
        ...
        **kwargs
        ...
        # Log new model size
        original_size = self.get_model_size(output_path.parent / "model.onnx") if (output_path.parent / "model.onnx").exists() else 0
        quantized_size = self.get_model_size(output_path / "model_quantized.onnx")
        
        if original_size > 0:
            compression_ratio = original_size / quantized_size
            logger.info(f"Quantized model size: {quantized_size:.1f} MB (compression ratio: {compression_ratio:.1f}x)")
        else:
    ...
    def optimize(
        ...
        sess_options.optimized_model_filepath = str(output_path / "model_optimized.onnx")
    ...
    def validate_conversion(self, original_model: Any, converted_model_path: Path) -> bool:
