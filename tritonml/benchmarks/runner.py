...
class BenchmarkRunner:
    ...
    def benchmark_dataset(self,
                         ...
                         runs_per_batch: int = 10) -> Dict[str, Any]:
        ...
        """
        # Load dataset
        dataset = dataset_loader.load(max_samples=num_samples)
        ...
        for batch_size in batch_sizes:
            ...
            batch_results = {
                ...
                "throughput": {
                    "samples_per_second": float(batch_size / (np.mean(latencies_array) / 1000)),
            ...
            logger.info(f"  Mean latency: {batch_results['latency_ms']['mean']:.2f} ms")
            logger.info(f"  P95 latency: {batch_results['latency_ms']['p95']:.2f} ms")
            logger.info(f"  Throughput: {batch_results['throughput']['samples_per_second']:.2f} samples/sec")
            
        self.results[dataset_loader.dataset_name] = results
        return results
    
    def benchmark_multiple_datasets(self,
    ...
    def save_results(self, output_path: Union[str, Path], format: str = "json") -> None:
        ...
        if format == "json":
            ...
            for dataset_name, dataset_results in self.results.items():
                ...
                for batch_key, batch_results in dataset_results.get("batch_results", {}).items():
                    row = {
                        ...
                        "throughput_samples_sec": batch_results["throughput"]["samples_per_second"]
                    }
                    rows.append(row)
    ...
    def print_summary(self):
        ...
        for dataset_name, results in self.results.items():
            ...
            print(f"\n  {'Batch Size':<12} {'Mean Latency':<15} {'P95 Latency':<15} {'Throughput':<20}")
            print("  " + "-"*62)
            
            for batch_key, batch_results in sorted(results.get("batch_results", {}).items()):
                batch_size = batch_results["batch_size"]
                ...
                print(f"  {batch_size:<12} {mean_latency:<15.2f} {p95_latency:<15.2f} {throughput:<20.2f}")
